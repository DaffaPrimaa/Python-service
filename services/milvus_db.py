...
# Hanya izinkan karakter aman untuk employee_id (UUID, alfanumerik, hypen, underscore)
_SAFE_ID_RE = re.compile(r'^[\w\-]{1,100}$')

def _validate_employee_id(employee_id: str) -> None:
    """Lempar ValueError jika employee_id mengandung karakter berbahaya."""
    if not _SAFE_ID_RE.match(employee_id):
        raise ValueError(f"employee_id tidak valid: '{employee_id}'")
...
class MilvusDB:
    ...
    def verify_specific(self, employee_id: str, embedding, threshold: float) -> dict:
        """Verifikasi 1:1 — bandingkan embedding foto dengan milik employee tertentu saja."""
        try:
            _validate_employee_id(employee_id)
        except ValueError as e:
            return {"success": False, "error": str(e)}
        ...
        try:
            ...
            if not results or not results[0]:
                return {"success": False, "error": f"Employee {employee_id} belum di-enroll"}

            top = results[0][0]
            sim = float(top.distance)

            if sim >= threshold:
                return {"success": True, "matched": True, "employee_id": employee_id, "similarity": sim}

            return {"success": True, "matched": False, "similarity": sim}
        except Exception as e:
            return {"success": False, "error": str(e)}
