...
@router.post("/verify-specific", tags=["Verify"])
async def verify_face_specific(
    employee_id: str = Form(...),
    image_url: str = Form(...),
    threshold: float = Form(None),
):
    try:
        ...
        verify_result = await anyio.to_thread.run_sync(
            db.verify_specific, employee_id, embedding, threshold
        )

        if not verify_result.get("success", False):
            return failure_to_response(verify_result)

        if not verify_result.get("matched", False):
            return {
                "success": True,
                "matched": False,
                "similarity": verify_result.get("similarity", 0.0),
                "threshold": threshold,
                "message": "Wajah tidak cocok dengan akun ini"
            }

        return {
            "success": True,
            "matched": True,
            "employee_id": verify_result["employee_id"],
